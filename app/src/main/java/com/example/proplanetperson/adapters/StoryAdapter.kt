...
import com.example.proplanetperson.StoryActivity // Correct import for StoryActivity
...
// import kotlinx.android.synthetic.main.fragment_profile.view.* // REMOVE THIS DEPRECATED IMPORT
    ...
    RecyclerView.Adapter<StoryAdapter.ViewHolder>()
{
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (holder.adapterPosition == 0)
        {
            // Access properties directly (no change needed here as you correctly used nullable '!!' previously)
        ...
        userRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists())
                {
                    ...
                    user?.let { // Use 'let' for safe access to 'user'
                        // Access properties directly
                        Picasso.get().load(it.image).placeholder(R.drawable.profile).into(viewHolder.story_image)

                        if (position != 0){
                            // Access properties directly
                            Picasso.get().load(it.image).placeholder(R.drawable.profile).into(viewHolder.story_image_seen)
                            // Access properties directly
                            viewHolder.story_user_name!!.text = it.username
                        }
                    }
                }
            }
            ...
            override fun onCancelled(error: DatabaseError) {
                // Handle cancellation, e.g., log the error
            }
        })
    }
            ...
            override fun onDataChange(datasnapshot: DataSnapshot) {
                ...
                if(click)
                {
                    if (counter > 0){
                        ...
                        {
                                dialog, _ -> // Use _ for unused parameter
                            val intent = Intent(mContent, StoryActivity::class.java)
